...
# в которую пишет коллектор, и обёрткой её подменять нельзя: через обёртку
# запрос читает колонку целиком, на стенде это 56 секунд против 0.7 при том,
# что смысл проверки — «насколько отстал коллектор», то есть речь именно о той
# таблице, в которую он пишет.
#
# Сам запрос ограничен окном последних минут (recent_five_minute_filter): после
# перехода на новую раскладку время стоит в конце ключа сортировки, поэтому
# max(time_received_ns) без такого окна проходит по всей сегодняшней таблице.
