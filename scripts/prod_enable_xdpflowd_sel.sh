#!/usr/bin/env bash
# prod_enable_xdpflowd_sel.sh — включить постоянный xdpflowd на sel:
#   * сохранить iptables backup + state для rollback
#   * снять единственное правило PREROUTING -j NETFLOW на зеркале
#   * остановить goflow2 контейнеры (список из /etc/xdpflowd/sel.env)
#   * установить systemd unit xdpflowd.service и запустить сервис
#
# Предусловия:
#   * собранный GrapesNTA в $REPO_ROOT (default: каталог репозитория относительно этого скрипта)
#   * /etc/xdpflowd/sel.env — скопирован из deploy/sel/xdpflowd.env.example и заполнен (CH DSN и т.д.)
#
# Запуск:
...
if [[ ! -r "$ENV_INSTALL" ]]; then
  ...
  install -m 0600 "$REPO_ROOT/deploy/sel/xdpflowd.env.example" "$ENV_INSTALL"
...
mkdir -p "$SPOOL_DIR" /etc/xdpflowd

# ---------- найти ровно одно правило ipt_NETFLOW на $IFACE ----------
RULE_TABLE=""
RULE_SPEC=""
...
IPT_BACKUP="/root/iptables-save-before-sel-permanent-$TS.txt"
...
{
  echo "# Written by prod_enable_xdpflowd_sel.sh at $TS"
  printf 'REPO_ROOT=%q\n' "$REPO_ROOT"
  printf 'IFACE=%q\n' "$IFACE"
  printf 'RULE_TABLE=%q\n' "$RULE_TABLE"
  printf 'RULE_SPEC=%q\n' "$RULE_SPEC"
...
sed "s|/opt/GrapesNTA|${ESC_REPO}|g" "$REPO_ROOT/deploy/sel/xdpflowd.service" > "$TMP_UNIT"
install -m 0644 "$TMP_UNIT" "/etc/systemd/system/$SYSTEMD_UNIT_NAME"
rm -f "$TMP_UNIT"
chmod 0755 "$REPO_ROOT/deploy/sel/xdpflowd-exec.sh" || true
