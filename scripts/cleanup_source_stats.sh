...
tables=(
  ...
  "traffic_asn_1m|minute|round(sum(bytes)/1e9, 3) AS gb"
  "traffic_asn_pair_1m|minute|round(sum(bytes)/1e9, 3) AS gb"
  "traffic_asn_1h|hour|round(sum(bytes)/1e9, 3) AS gb"
  "traffic_asn_pair_1h|hour|round(sum(bytes)/1e9, 3) AS gb"
