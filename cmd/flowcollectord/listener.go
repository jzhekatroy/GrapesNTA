...
func (l *udpListener) Run(ctx context.Context) error {
	...
	for i := 0; i < l.readers; i++ {
		...
		if l.readBuf > 0 {
			if err := pc.SetReadBuffer(l.readBuf); err != nil {
				l.log.Warn(l.name+" set udp read buffer", "reader", i, "requested_bytes", l.readBuf, "err", err)
			}
		}
		readers = append(readers, pc)
	}

	l.log.Info(l.name+" listener started",
		"addr", l.addr,
	...
	datagrams := make(chan udpDatagram, l.queueSize)
...
func (l *udpListener) runReader(ctx context.Context, id int, pc *net.UDPConn, datagramBufSize int, datagrams chan<- udpDatagram) error {
	...
	for {
		...
		if err != nil {
			...
			l.log.Warn(l.name+" read", "reader", id, "err", err)
			return err
		}
		cp := make([]byte, n)
		copy(cp, buf[:n])
		...
		d := udpDatagram{b: cp, receivedAt: time.Now().UTC(), src: src}
		select {
		case datagrams <- d:
		default:
			l.parser.addUDPQueueDrop()
		}
	}
}

func (l *udpListener) runWorker(ctx context.Context, datagrams <-chan udpDatagram) {
...
func (l *udpListener) LogMetrics() {
	if l == nil || l.parser == nil {
		return
	}
	l.parser.logMetrics(l.log)
}

func (l *udpListener) receiverMetrics() flowingest.ReceiverMetrics {
	if l == nil || l.parser == nil {
		return flowingest.ReceiverMetrics{}
	}
	return l.parser.receiverMetrics()
}

func sumReceiverMetrics(items []flowingest.ReceiverMetrics) flowingest.ReceiverMetrics {
	var out flowingest.ReceiverMetrics
	for _, m := range items {
		out.Datagrams += m.Datagrams
		out.RecordsParsed += m.RecordsParsed
		out.ParseErrors += m.ParseErrors
		out.UDPQueueDrops += m.UDPQueueDrops
	}
	return out
}
