#!/usr/bin/env bash
# Wrapper for systemd: load /etc/xdpflowd/xdpflowd.env and exec xdpflowd.
# Override env path with XDPFLOWD_ENV_FILE if needed.

set -euo pipefail

ENV_FILE="${XDPFLOWD_ENV_FILE:-/etc/xdpflowd/xdpflowd.env}"
if [[ ! -r "$ENV_FILE" ]]; then
...
# Flow drain strategy: timer (idle/active per-key) | batch (whole-map
# BPF_MAP_LOOKUP_AND_DELETE_BATCH every XDP_DRAIN_INTERVAL). batch is far faster
# at high pps and prevents flow-map overflow (map_full). With batch, the drain
# interval is the effective active timeout.
XDP_DRAIN_MODE="${XDP_DRAIN_MODE:-timer}"
XDP_DRAIN_INTERVAL="${XDP_DRAIN_INTERVAL:-0s}"

# Userspace flow aggregation (drain-mode=batch only). Folds the frequent BPF
# drain slices into one row per flow per active window, so ClickHouse/NetFlow
# get ~1 row per real flow instead of one per drain tick. With it ON, keep
# XDP_DRAIN_INTERVAL small (e.g. 5s) and set XDP_AGG_ACTIVE high (e.g. 60s):
# the row-volume reduction factor ≈ XDP_AGG_ACTIVE / XDP_DRAIN_INTERVAL.
