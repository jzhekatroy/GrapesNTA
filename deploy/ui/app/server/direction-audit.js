...
function listInterfacesByTraffic({
  ...
  const switchIpTrimmed = String(switchIp || '').trim();
  const switchFilterSql = switchIpTrimmed ? 'WHERE switch_ip = {switch_ip:String}' : '';
  const outerFilterSql = onlyUnmarked ? `${switchFilterSql ? 'AND' : 'WHERE'} boundary = 'unknown'` : '';
  ...
  return {
    sql: `
      ...
      FROM (
        SELECT
          *,
          sum(bytes) OVER () AS stats_total_bytes,
          sumIf(bytes, boundary != 'unknown') OVER () AS stats_classified_bytes,
          countIf(boundary = 'unknown') OVER () AS stats_unmarked_ports
        FROM (
          SELECT
          ...
          FROM (
            ...
            FROM (
              SELECT
                ${flowSamplerIpExpr(`f.${c.samplerCol}`)} AS switch_ip,
    ...
    params: {
      hours: lookbackHours,
      limit: lim,
      asn_threshold: threshold,
      ...(switchIpTrimmed ? { switch_ip: switchIpTrimmed } : {}),
    },
    meta: { lookbackHours, asnThreshold: threshold, switchIp: switchIpTrimmed || null },
    map(rows) {
      const stats = rows.length ? {
        totalBytes: Number(rows[0].stats_total_bytes) || 0,
        classifiedBytes: Number(rows[0].stats_classified_bytes) || 0,
        unmarkedPortsWithTraffic: Number(rows[0].stats_unmarked_ports) || 0,
      } : {
        totalBytes: 0,
        classifiedBytes: 0,
        unmarkedPortsWithTraffic: 0,
      };
      const classifiedPercent = stats.totalBytes
        ? Math.round((stats.classifiedBytes * 10000) / stats.totalBytes) / 100
        : 0;
      return {
        rows: rows.map((r) => ({
