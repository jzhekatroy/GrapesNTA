...
function buildObservationJobs(all, rollupById, lastTick) {
  ...
  const candidates = all.filter((o) => o.live?.enabled || o.materialize?.enabled);
  ...
  return candidates.map((o) => {
    ...
    let skipReason = null;
    if (liveEnabled && scope.materializeRequired && !materializeEnabled) {
      skipReason = 'live включён, но materialize выключен — воркер не возьмёт job';
    ...
    } else if (!materializeEnabled && !liveEnabled) {
      skipReason = 'live и materialize выключены';
    ...
    const rank = (j) => {
      ...
      if (j.skipReason && j.liveEnabled) return 1;
...
function buildProblems({ worker, jobs, trafficRows, trafficError }) {
  ...
  for (const j of jobs) {
    if (j.liveEnabled && j.scope.materializeRequired && !j.materializeEnabled) {
      problems.push(problem(
        'warning',
        'obs_live_without_materialize',
        `«${j.name}»: live включён, materialize выключен — данные не материализуются`,
        { observationId: j.id },
      ));
    }
...
async function getWorkerDiagnostics() {
  ...
  const liveOrMatIds = all
    .filter((o) => o.live?.enabled || o.materialize?.enabled)
    .map((o) => o.id);
  ...
  // Only actionable rows: worker picks them, or live is on but misconfigured.
  const jobs = allJobs.filter((j) => j.workerWillPick || (j.liveEnabled && j.skipReason));
